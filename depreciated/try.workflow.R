...
# args will be cahin number / info about the type of run
...
# New number of iterations because it needs thinning
n.iter <- 600000
thin <- 40

model = "models/multivariate.grp.model.txt"
pft.save <- c()
if(runs$multi.pft){ # Without na's
  print("-------------------------")
  print(model)
  print(Sys.time())
  j.data <- try
...
if(runs$multi.pft.na){ # With na's 
  print("-------------------------")
  print(model)
  print(Sys.time())
  j.data <- try.na
