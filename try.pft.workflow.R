...
vname <- "Sigma"
for(k in 1:Ngroup){
  S <- as.data.frame(matrix(NA,Nvars,Nvars))
  for(i in 1:Nvars){
    for(j in 1:Nvars){
      idx <- which(colnames(out.Sigma)==paste0(vname,"[",k,",",i,",",j,"]"))
      S[i,j] <- mean(out.Sigma[,idx])
...
X1.g <- as.numeric(colMeans(X.g[,1:Nobs]))
X2.g <- as.numeric(colMeans(X.g[,(Nobs+1):(2*Nobs)]))
X3.g <- as.numeric(colMeans(X.g[,(2*Nobs+1):(3*Nobs)]))

save(X.g, file = "X_group.Rdata")

par(mfrow=c(2,2))
qqplot(try.pft.na[[1]],X1.g)
abline(0, 1, lty=2,  col = 'red')
qqplot(try.pft.na[[2]],X2.g)
abline(0, 1, lty=2,  col = 'red')
qqplot(try.pft.na[[3]],X3.g)
abline(0, 1, lty=2,  col = 'red')
